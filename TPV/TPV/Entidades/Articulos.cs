...
namespace TPV.Entidades
{
    class Articulos
    {
        ...
        public DataTable GetListaPrecios()
        {
            ...
            {
                db.Connect();
                db.CreateCommand("SELECT codigo, descripcion, importe_costo, iva, margen, importe_venta, rubro, articulo_id FROM articulos LEFT JOIN rubros on rubros.rubro_id = articulos.rubro_id WHERE articulos.borrado = 0 ORDER BY articulo_id DESC");
                dt = db.GetDataTable();
                db.DisConnect();
            }
            catch (Exception ex)
